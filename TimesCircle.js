...
class TimesCircle {
    ...
    resize(r, lineDensity) {
        ...
        for (let n = 0; n < this.num;) {
            const angle = (n / this.num) * Math.twoPI;
    ...
    draw(ctx, colorLoop) {
        ...
        ctx.fillStyle = "#fff";
        ctx.moveTo(1, 0);
        ctx.arc(0, 0, 1, 0, Math.twoPI);
        ctx.fill();

        ctx.beginPath(); // math lines
        ctx.lineWidth = this.lineWidth;
        ctx.strokeStyle = `hsl(${this.mult / colorLoop * 360}, 100%, 50%)`;
        for (let n = 0; n < this.num;) {
            const angle = (n * this.mult / this.num % 1) * Math.twoPI;
        ...
        ctx.lineWidth = 2;
        ctx.strokeStyle = "#fff";
        ctx.arc(0, 0, this.r, 0, Math.twoPI);
        ctx.stroke();
    }
}
